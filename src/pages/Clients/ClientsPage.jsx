                ...
                <tbody>
                  {filtered.map((c, i) => (
                    <tr key={c.id} className={`prod-tr ${i % 2 !== 0 ? "prod-tr-alt" : ""}`}>
                      <td className="prod-td"><span className="prod-name">{c.name}</span></td>
                      ...
                      <td className="prod-td prod-td-center">
                        <div className="prod-actions">
                          <button className="prod-btn-edit" onClick={() => openEdit(c)}>Editar</button>
                          <button className="prod-btn-delete" onClick={() => setDeleteId(c.id)}>Eliminar</button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="prod-modal-overlay" onClick={closeModal}>
          <div className="prod-modal" onClick={e => e.stopPropagation()}>
            <div className="prod-modal-header">
              <h2 className="prod-modal-title">{editing ? "Editar cliente" : "Nuevo cliente"}</h2>
              <button className="prod-modal-close" onClick={closeModal}>✕</button>
            </div>
            <div className="prod-modal-body">
              <div className="prod-field">
                <label className="prod-label">Nombre</label>
                <input className="prod-input" placeholder="Nombre del cliente"
                  value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              </div>
              <div className="prod-field">
                <label className="prod-label">Email</label>
                <input className="prod-input" type="email" placeholder="correo@ejemplo.com"
                  value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
              </div>
              <div className="prod-field">
                <label className="prod-label">Teléfono</label>
                <input className="prod-input" placeholder="+56 9 1234 5678"
